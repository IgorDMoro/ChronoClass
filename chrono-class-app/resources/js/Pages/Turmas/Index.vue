                            ...
                            <table class="min-w-full">
                                <thead>
                                    <tr class="border-b border-neutral-700">
                                        ...
                                        <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Período</th>
                                        <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Ano Letivo</th>
                                ...
                                <tbody>
                                    <tr v-for="turma in turmas" :key="turma.id" class="border-b border-neutral-800 hover:bg-white/5 transition-colors duration-150">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-200">{{ turma.nome }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{{ turma.periodo }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{{ turma.ano_letivo }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end gap-x-6">
                                                <div class="relative group">
                                                    <Link :href="route('turmas.edit', turma.id)" class="text-gray-400 hover:text-orange-400 transition-colors duration-150">
                                                ...
                                                <div class="relative group">
                                                    <button @click.prevent="openConfirmModal(turma)" class="text-gray-500 hover:text-red-500 transition-colors duration-150 focus:outline-none">
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
        ...
        <div v-if="showConfirmDeleteModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div class="bg-neutral-800 max-w-lg w-full rounded-lg shadow-2xl ring-1 ring-white/10 p-6 sm:p-8">
                ...
                <p class="mt-2 text-sm text-gray-400">
                    Tem certeza que deseja excluir a turma <strong class="text-orange-400">{{ turmaToDelete.nome }}</strong>? Esta ação não pode ser desfeita.
                </p>
