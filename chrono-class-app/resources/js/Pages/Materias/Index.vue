                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-lg font-medium text-gray-900">Lista de Matérias</h3>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Comp. Tipo</th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ensw. Tipo</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="materia in materias" :key="materia.id">
                                    <td class="px-6 py-4 whitespace-nowrap">{{ materia.codigo }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ materia.nome }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ materia.carga_horaria }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ materia.modalidade }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ materia.comp_tipo }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ materia.ensw_tipo }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <Link :href="route('materias.edit', materia.id)" class="text-indigo-600 hover:text-indigo-900 mr-4">Editar</Link>
                                        <button @click="deleteMateria(materia.id)" class="text-red-600 hover:text-red-900">Excluir</button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>

                        <h3 class="text-lg font-medium text-gray-900 mt-8 mb-4">Importar Matérias da Planilha</h3>
                        <form @submit.prevent="submitImport" enctype="multipart/form-data" class="flex flex-col space-y-4">
                            <div>
                                <label for="fileInput" class="block text-sm font-medium text-gray-700">Selecione o arquivo Excel/CSV:</label>
                                <input type="file" @change="handleFileUpload" id="fileInput" required
...
const deleteMateria = (id) => {
    if (confirm('Tem certeza que deseja excluir esta matéria?')) {
        ...
        deleteForm.post(route('materias.destroy', id), {
            ...
            onError: (errors) => {
                alert('Erro ao excluir matéria: ' + (errors.message || 'Verifique o console.'));
