...
const props = defineProps({
    sala: Object, // A sala que será editada, vindo do Controller
});

const form = useForm({
    _method: 'put', // Importante para que o Inertia envie uma requisição PUT
...
const submit = () => {
    form.post(route('salas.update', props.sala.id));
    // form.put(route('salas.update', props.sala.id)); // Alternativa mais direta para PUT
};
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar Sala: {{ form.nome }}</h2>
        </template>
