...
const dropTarget = ref(null);  // gradeId sendo hovado

const onDragStart = (horario, gradeId) => {
    dragInfo.value = { horarioId: horario.id, fromGradeId: gradeId };
};

const onDragOver = (gradeId, event) => {
    event.preventDefault();
    if (dragInfo.value && dragInfo.value.fromGradeId !== gradeId) {
        dropTarget.value = gradeId;
    }
};
...
const onDrop = (toGradeId) => {
    ...
    // Horários que ocupam o mesmo slot no destino (candidatos a swap)
    ...
    if (slotDestino.length > 0) {
        // ── SWAP: slot destino ocupado → troca os dois conjuntos de aulas ──

        // Valida: o professor que entra não pode já estar no destino fora do que vai sair
        ...
        // Valida: os professores que saem do destino não podem conflitar na origem
        for (const h of slotDestino) {
            const conflitoOrigem = fromGrade.horarios.find(hf =>
                hf.dia_semana    === h.dia_semana &&
                hf.horario_bloco === h.horario_bloco &&
        ...
        // Executa o swap localmente
        fromGrade.horarios.splice(horarioIdx, 1);                        // retira arrastado da origem
        slotDestino.forEach(h => {                                        // retira do destino e coloca na origem
            const idx = toGrade.horarios.findIndex(hh => hh.id === h.id);
            if (idx !== -1) toGrade.horarios.splice(idx, 1);
            fromGrade.horarios.push({ ...h });
        });
        toGrade.horarios.push({ ...horario });                           // coloca arrastado no destino

        // Registra todas as movimentações do swap como pendentes
        registrarPendente(horario, fromGradeId, fromGrade.nome, toGradeId, toGrade.nome);
        slotDestino.forEach(h =>
            registrarPendente(h, toGradeId, toGrade.nome, fromGradeId, fromGrade.nome)
        );

    } else {
        // ── MOVE SIMPLES: slot destino vazio ──

        // Valida conflito de professor no destino
                        ...
                        >
                            ...
                            <div class="p-3 border-b border-gray-200 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-800 rounded-t-lg">
                                ...
                                <div class="flex gap-1.5 mt-1.5 flex-wrap">
                                    <span v-if="grade.turma" class="text-[10px] px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-300 font-medium">
                                        {{ grade.turma.nome }}
                                    </span>
                                ...
                                <p class="text-[10px] text-gray-400 dark:text-gray-500 mt-1">
                                    {{ grade.horarios.length }} horário{{ grade.horarios.length !== 1 ? 's' : '' }}
                                </p>
                            </div>
                            ...
                            <div class="p-2 space-y-1.5 min-h-[120px]">
                                <template v-for="dia in diasDaSemana" :key="dia">
                                    <template v-for="bloco in horariosBlocos" :key="bloco">
                                        ...
                                        >
                                            <p class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                                {{ diaLabel(dia) }} · {{ blocoLabel(bloco) }}
                                            </p>
                                            <p class="text-xs font-semibold text-gray-800 dark:text-gray-200 leading-snug mt-0.5">
                                                {{ horario.materia?.nome || '—' }}
                                            </p>
                                            <p class="text-[10px] text-gray-500 dark:text-gray-400 truncate">
                                                {{ horario.professor?.nome || '—' }}
                                            </p>
                                            <p v-if="horario.sala" class="text-[10px] text-gray-400 dark:text-gray-500">
                                                🚪 {{ horario.sala }}
                                            </p>
                                        </div>
                                    </template>
                                ...
                                <!-- Coluna vazia -->
                                <div v-if="grade.horarios.length === 0" class="text-center py-8 text-gray-300 dark:text-gray-600 text-xs select-none">
                                    Solte aqui
                                </div>
                            </div>
                        </div>
